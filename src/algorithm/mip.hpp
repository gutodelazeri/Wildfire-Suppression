...
public:
    MIP(Instance &_I, GeneralParameters _generalParameters, MipParameters _mipParameters) : I(_I), B(_I, true, false), model(nullptr),
                                                                                            generalParameters(_generalParameters), mipParameters(_mipParameters)
    {
    ...
    void write_statistics(ostream &out)
    {
        ...
        for (const auto &[time, objective, bound] : model->get_trajectory())
            traj = traj + "(" + to_string(time) + "," + to_string(round(objective)) + "," + to_string(round(bound)) + "), ";
        if (traj.size() > 2)
        {
            traj.pop_back();
            traj.pop_back();
        ...
        traj = traj + "]\"";
        out << I.get_instance_id() << ","
            << generalParameters.seed << ","
            << model->get_objv() << ","
            << model->get_elapsed_time() << ","
            << B.time_to_survival << ","
            << (B.burnt_vertices == INF_VERTEXID ? "-" : to_string(B.burnt_vertices)) << ","
            << model->get_lower_bound() << ","
            << model->get_mip_gap() << ","
            << model->get_node_count() << ","
            << model->get_solution_count() << ","
            << model->get_open_node_count() << ","
            << model->get_num_constr() << ","
            << model->get_num_vars() << ","
            << model->get_num_bin_vars() << ","
            << model->get_num_nzs() << ","
            << model->get_max_coeff() << ","
            << model->get_min_coeff() << ","
            << model->get_max_bound() << ","
            << model->get_min_bound() << ","
            << model->get_max_obj_coeff() << ","
            << model->get_min_obj_coeff() << ","
            << model->get_max_rhs() << ","
            << model->get_min_rhs() << ","
            << model->presolve_get_num_constr() << ","
            << model->presolve_get_num_vars() << ","
            << model->presolve_get_num_bin_vars() << ","
            << model->presolve_get_num_nzs() << ","
            << model->presolve_get_max_coeff() << ","
            << model->presolve_get_min_coeff() << ","
            << model->presolve_get_max_bound() << ","
            << model->presolve_get_min_bound() << ","
            << model->presolve_get_max_obj_coeff() << ","
            << model->presolve_get_min_obj_coeff() << ","
            << model->presolve_get_max_rhs() << ","
            << model->presolve_get_min_rhs();
