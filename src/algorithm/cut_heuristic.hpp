...
private:
    ...
    unsigned feasible_iterations; // Number of iterations in which a feasible solution was found
    ...
    TIME earliest_feasible_solution; // Earliest binary search time at which a feasible solution was found
...
public:
    CutHeuristic(Instance &I, GeneralParameters general, CutHeuristicParameters cuth) : clock(Clock(false)),  total_iterations(0), elapsed_time(0), feasible_iterations(0), iterations_to_best(0), time_to_best(0), best_time(INF_TIME),
                                                                                                              earliest_feasible_solution(INF_TIME), I(I), generalParameters(general), cuthParameters(cuth), A0(I, true, true), B(I)
    {
    ...
    void find_cut(Solution& S, TIME t_start, vector<VERTEXID>& P) {
        ...
        for(RESOURCE i : I.get_R())
            num_resources += I.get_capacity(i);
        CutModel model(S, t_start, num_resources);
        P.clear();
        model.optimize(P);
    }
    ...
    void run()
    {
        ...
        } while(abs(t_lb - t_ub) > 0.01 && clock.now() < generalParameters.timelimit);
