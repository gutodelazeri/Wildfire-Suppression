...
protected:
    ...
    // 4 - A burnt vertex cannot receive a resource
    ...
    { 
        for (RESOURCE i : I.get_R())
        {
            ...
            for (VERTEXID v : I.get_V()) {
                model->addConstr(a[v] >= r[{i, v}] * release_time, "C5." + to_string(i) + "_" + to_string(v));
            }
        }
    }
...
public:
    BasicModel(Instance &_I, GeneralParameters gp, MipParameters mp) : I(_I), generalParameters(gp), mipParameters(mp)
    {
