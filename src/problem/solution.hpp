...
public:
    ...
    // Resource allocation
    string fingerprint; // A string indicating the delay associated with each vertex (a solution can be fully characterized by the amount of delay added to each vertex). Check Instance::create_delay_mappings() for a better understanding of this.
    ...
    bool partial_solution; // Indicates whether the solution is partial (i.e., it does not contain the fire propagation information) or not
    // Statistics
    ...
    unsigned iter; // Iteration number of the solution (only updated by some algorithms)


    Solution(Instance &_I, bool initialize = false, bool initialize_feasible = false) : I(_I), objv(INF_VALUE), lb(0), burnt_vertices(INF_VERTEXID), time_to_survival(INF_TIME), timestamp(0), iter(0)
    {
        ...
        if (initialize) // Allocate space 
        {
            ...
            if (initialize_feasible) // Initialize a feasible solution (empty allocation)
            {
    ...
    inline TIME get_vertex_delay(VERTEXID v) const
    {
        return I.get_delay_value(fingerprint[v]);
    }

    inline bool has_resource(VERTEXID v) const
    {
        return fingerprint[v] != Instance::NULL_DELAY;
    }

    inline bool has_resource(VERTEXID v, RESOURCE i) const
    {
        return find(protected_vertices[i].begin(), protected_vertices[i].end(), v) != protected_vertices[i].end();
    }

    inline void set_base_location(RESOURCE i, VERTEXID b)
    {
        base_location[i] = b;
    }

    inline void set_deployment_time(RESOURCE i, TIME t)
    {
        deployment_time[i] = t;
    }

    inline CAPACITY used_capacity(RESOURCE i) const
    {
        return protected_vertices[i].size();
    }
    ...
    void allocate_resource(VERTEXID v)
    {
        fingerprint[v] = I.get_delay_ID(0);
    }
    ...
    void deallocate_resource(VERTEXID v)
    {
        fingerprint[v] = Instance::NULL_DELAY;
    }

    inline string get_fingerprint()
    {
        return fingerprint;
    }
    
    void update() {
        compute_shortest_path_forest(*this);
    }
    ...
    bool is_trivial_solution() {        
        for(RESOURCE i : I.get_R())
            if(protected_vertices[i].size() > 0)
                return false;
        return true;
    }
    ...
    static void load_solution(const string &file, Solution &S)
    {
        ...
        for (auto &[key, value] : sol["allocation"].items())
        {
            ...
            {
                cout << "Resource " << i << " (range " << S.I.get_range(i) << ") does not have a base location." << endl;
                exit(1);
            }
    ...
    static void write_solution(ostream &fout, Solution &S)
    {
        ...
        for (RESOURCE i : I.get_R())
        {
            if (I.get_range(i) < INF_DISTANCE)
                data["allocation"][to_string(i)] = {{"time", S.deployment_time[i]}, {"base", S.base_location[i]}, {"protected", json::array()}};
            else
                data["allocation"][to_string(i)] = {{"time", S.deployment_time[i]}, {"base", "NA"}, {"protected", json::array()}};
        ...
        for (int x = x_range.first; x <= x_range.second; x += xy_dist)
        {
            for (int y = y_range.first; y <= y_range.second; y += xy_dist)
            {
                ...
                if(v == INF_VERTEXID)
                    ...
                    fout << "0";
                else if (S.fire_arrival_time[v] < I.get_optimization_horizon())
                    fout << "x";
                else
                    fout << ".";
            }
    ...
    static bool check_feasibility(Solution &S)
    {
        ...
        // Save the reported objective value, time to survival, and fire arrival time
        ...
        for (VERTEXID v : I.get_Vf())
        {
            ...
            {
                cout << "Reported fire arrival time at vertex " << v << " is exceeds correct fire arrival time." << endl;
        ...
        if (ff(tmp_obvj) != ff(S.objv))
        {
            ...
            if (ff(tmp_obvj) < ff(S.objv))
                exit(1);
            // The solver may report a higher objective value than the correct one by setting y_v variables to 1 even though the associated vertex is not burnt.
        }
        ...
        for (RESOURCE i : I.get_R())
        {
            ...
            {
                cout << "Resource " << i << " protected " << num_protected << " vertices without being deployed." << endl;
                exit(1);
            }
            if (deployment_time == INF_TIME)
                continue;
            ...
            if (range == INF_DISTANCE)
            {
                ...
                {
                    cout << "Resource " << i << " has a base location without having a maximum range." << endl;
                    exit(1);
                }
            }
            else 
            {
                ...
                {
                    cout << "Vertex " << base_location << " cannot be the base location of resource " << i << endl;
                    exit(1);
                }
                // Safety constraints
                for(VERTEXID v :  I.get_Vf()) {
                    if(ff(I.distance(v, base_location)) <=  ff(safety_distance_base_location)) {
                        if(ff(S.fire_arrival_time[v]) < ff(deployment_time + safety_time_base_location)){
                            cout << "A vertex in the safety radius of the base location of resource " << i << " does not respect the safety time." << endl;
                            cout << "       Base location: " << base_location << " (" << I.get_vertex_signature(base_location) << ")" << endl;
                            cout << "       Safety time: " << safety_time_base_location << endl;
                            cout << "       Safety distance: " << safety_distance_base_location << endl;
                            cout << "       Problematic vertex: " << v << " (" << I.get_vertex_signature(v) << ")" << endl;
                            cout << "       Fire arrival time to the base location: " << S.fire_arrival_time[base_location] << endl;
                            cout << "       Fire arrival time to " << v << ": " << S.fire_arrival_time[base_location] << endl;
                            cout << "       Distance between the vertices: " << I.distance(v, base_location) << endl;
                            exit(1);
                        }
                    }
                }
            ...
            for (VERTEXID v : protected_vertices)
            {
                ...
                if (range != INF_DISTANCE && ff(range) < ff(I.distance(v, base_location)))
                {
                    ...
                    cout << "       Distance from protected vertex " << v << " to the base location " << base_location << ": " << S.I.distance(v, base_location) << endl;
                ...
                for(VERTEXID u :  I.get_Vf()) {
                    if(ff(I.distance(v, u)) <=  ff(safety_distance_protected_vertices)) {
                        if(ff(S.fire_arrival_time[u]) < ff(deployment_time + safety_time_protected_vertices)){
                            cout << "A vertex in the safety radius of the protected vertex " << v << " does not respect the safety time." << endl;
                            cout << "       Resource: " << i << endl;
                            cout << "       Protected vertex: " << v << " (" << I.get_vertex_signature(v) << ")" << endl;
                            cout << "       Safety time: " << safety_time_protected_vertices << endl;
                            cout << "       Safety distance: " << safety_distance_protected_vertices << endl;
                            cout << "       Problematic vertex: " << u << " (" << I.get_vertex_signature(u) << ")" << endl;
                            cout << "       Fire arrival time to the protected vertex: " << S.fire_arrival_time[v] << endl;
                ...
                // Check if the internal data structures of S assign the correct delay value to vertex v
    ...
    static void compute_shortest_path_forest(Solution &S)
    {
        ...
        if (!heap.is_initialized())
            heap.initialize(I.get_num_vertices());
        heap.clear();
        if (S.fire_arrival_time.size() < I.get_num_vertices())
            S.fire_arrival_time.resize(I.get_num_vertices());
        if (S.predecessor.size() < I.get_num_vertices())
            S.predecessor.resize(I.get_num_vertices());
    ...
    static void update_subtree(Solution &S, ranges::input_range auto&& protected_vertices, vector<tuple<VERTEXID, VERTEXID, TIME>> &affected_vertices)
    {
        /*
            Assumptions:
        ...
        */
        ...
        for (VERTEXID u : protected_vertices)
            for (auto &[v, t_uv] : I.get_outgoing_arcs(u))
                if (S.predecessor[v] == u)
                    heap.insertElement(v, S.fire_arrival_time[v]);
        // Identify all affected vertices in order of fire arrival time. Set their fire arrival times to infinity
        while (!heap.empty())
        {
            VERTEXID u = heap.findAndDeleteMinElement();
            ...
            for (auto &[v, t] : I.get_incoming_arcs(u))
            {
                ...
                if (ff(S.fire_arrival_time[v]) < ff(INF_TIME) && ff(S.fire_arrival_time[u]) == ff(S.fire_arrival_time[v] + t_vu))
                {
        ...
        // Update fire arrival time of vertices that are in the border between the subtree of affected vertices and the rest of the shortest-path tree
        for (VERTEXID u : Q)
        {
            for (auto &[v, t] : I.get_incoming_arcs(u))
            {
            ...
            if (ff(S.fire_arrival_time[u]) != ff(INF_TIME))
                heap.adjustHeap(u, S.fire_arrival_time[u]);
        }
    ...
    static void allocate_resources(Solution &S, RESOURCE i, ranges::input_range auto&& C)
    {
        ...
        {
            if (S.fire_arrival_time[u] >= H && a_u < H)
            {
                S.objv -= I.get_value(u);
                S.burnt_vertices--;
            }
    ...
    static void compute_heuristic_values(Solution &S, RESOURCE i, vector<VERTEXID> &C, VALUE &objv, TIME &time_to_survival)
    {
        Instance &I = S.I;
        TIME H = I.get_optimization_horizon();
        vector<tuple<VERTEXID, VERTEXID, TIME>> affected_vertices;
    ...
    static bool compute_greedy_schedule(Solution &S, vector<VERTEXID> P) {
        ...
        sort(R.begin(), R.end(), [&](RESOURCE i, RESOURCE j) {return I.get_release_time(i) > I.get_release_time(j);
        });
        sort(P.begin(), P.end(), [&](VERTEXID u, VERTEXID v) {return (S.fire_arrival_time[u] > S.fire_arrival_time[v]) || (S.fire_arrival_time[u] == S.fire_arrival_time[v] && u > v);});
        int P_idx = P.size() - 1;
        int R_idx = R.size() - 1;
        bool feasible = true;
        while(P_idx >= 0) {
            VERTEXID v = P[P_idx];
            RESOURCE i = R[R_idx];
            if(ff(S.fire_arrival_time[v]) < ff(I.get_release_time(i)))
                feasible = false;
