...
struct Vertex
{
    ...
    Vertex() = default;
    void add_incoming_arc(VERTEXID v, TIME t)
    {
        incoming.push_back({v, t});
    }
    void add_outgoing_arc(VERTEXID v, TIME t)
    {
        outgoing.push_back({v, t});
    }
};
...
public:
    ...
    TIME get_arc_cost(VERTEXID u, VERTEXID v)
    {
        return arc_costs[{u, v}];
    }

    auto get_outgoing_arcs(VERTEXID u)
    {
        bool is_blocked_u = blocked_vertex.find(u) != blocked_vertex.end();
        return vertices[u].outgoing | std::views::filter([this, u, is_blocked_u](pair<VERTEXID, TIME> arc)
                                                         {
    ...
    auto get_incoming_arcs(VERTEXID v)
    {
        ...
        return vertices[v].incoming | std::views::filter([this, v, is_blocked_v](pair<VERTEXID, TIME> arc)
                                                         {
    ...
    void restore_vertex(VERTEXID u)
    {
        blocked_vertex.erase(u);
    }

    void restore_arc(VERTEXID u, VERTEXID v)
    {
        blocked_arc.erase({u, v});
    }

    void block_vertex(VERTEXID u)
    {
        blocked_vertex.insert(u);
    }

    void block_arc(VERTEXID u, VERTEXID v)
    {
        blocked_arc.insert({u, v});
    }

    void restore_vertices()
    {
        blocked_vertex.clear();
    }
