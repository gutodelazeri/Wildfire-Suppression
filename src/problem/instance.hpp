...
private:
    ...
    DISTANCE manhattan2D(COORDINATE a, COORDINATE b)
    {
    ...
    void load_vertices_count(const json& instance) {
        num_vertices = instance["|V|"].get<VERTEXID>();
    }

    void load_optimization_horizon(const json& instance) {
        H = instance["H"].get<TIME>();
    }

    void load_ignition_vertices(const json& instance) {
        I = instance["I"].get<vector<VERTEXID>>();
    }
    ...
    void load_resources(const json& instance) {
        for (RESOURCE r = 0; r < instance["|R|"].get<RESOURCE>(); r++)
            R.push_back(r);
    }
    ...
    void load_base_locations(const json& instance) {
        ...
        {
            for (const auto &S : instance["Vb"])
                Vb.push_back(S.get<vector<VERTEXID>>());
            no_Vb = false;
        }
    }
    ...
    void load_protected_vertices(const json& instance) {
        ...
        {
            for (const auto &S : instance["Vp"])
                Vp.push_back(S.get<vector<VERTEXID>>());
            no_Vp = false;
        }
    }
    ...
    void load_release_times(const json& instance) {
        t = instance["t"].get<vector<TIME>>();
    }

    void load_resource_capacity(const json& instance) {
        c = instance["c"].get<vector<CAPACITY>>();
    }
    ...
    void load_delay_values(const json& instance) {
        delta = instance["delta"].get<vector<TIME>>();
    }
    ...
    /*
        - Since there is a finite number of resources, each with an associated delay value, there's a finite number of distinct delay values.
        - We create a mapping between these delay values and a unique ID (DELAYID) for each resource.
        - It's possible to show that a solution to the problem is completely determined by the delay assigned to each vertex. 
        - We represent solutions by a vector of characters (DELAYID) of size |V|, where each character corresponds to the delay assigned to the vertex. 
        - There's a special character NULL_DELAY, which indicates that the vertex is not protected by any resource.
    */
    void create_delay_mappings() {
        ...
        for (RESOURCE i : R)
            resID_2_delayID[i] = delayValue_2_delayID[delta[i]];
    }
    ...
    // Check whether the distance between vertices in the xy plane is always the same.
        ...
        for (VERTEXID u : get_V())
        {
            for (auto &[v, t_uv] : G.get_outgoing_arcs(u))
            {
                ...
                else if (dist != d)
                {
                    ...
                    cout << "	Coordinates: " << coordinates[u].to_string() << ", " << coordinates[v].to_string() << endl;
                    return INF_DISTANCE;
                }
            }
        }
...
public:
    ...
    bool is_basic_problem(){
        // Check if all decision points are distinct, i.e., vector t has no repeated elements
        ...
        if(!no_Vb || !no_Vp)
            return false;
        // Check that all resources have an infinite range, infinite expiration time, and zero safety time
        for(RESOURCE i : R){
    ...
    auto get_outgoing_arcs(VERTEXID v)
    {
        return G.get_outgoing_arcs(v);
    }

    auto get_incoming_arcs(VERTEXID v)
    {
        return G.get_incoming_arcs(v);
    }

    inline TIME get_arc_cost(VERTEXID u, VERTEXID v)
    {
        return G.get_arc_cost(u, v);
    }

    const vector<VERTEXID> &get_Vf()
    {
        return Vf;
    }
    ...
    const vector<VERTEXID> &get_I()
    {
        return I;
    }

    const vector<VERTEXID> &get_S()
    {
        return S;
    }

    const vector<RESOURCE> &get_R()
    {
        return R;
    }

    string get_vertex_signature(VERTEXID u)
    {
        return coordinates[u].to_string();
    }
    ...
    bool inI(VERTEXID v)
    {
        return binary_search(I.begin(), I.end(), v);
    }

    inline string get_instance_id()
    {
        return instance_id;
    }

    inline VERTEXID get_num_vertices()
    {
        return num_vertices;
    }

    inline RESOURCE get_num_resources()
    {
        return R.size();
    }

    inline DISTANCE get_range(RESOURCE i)
    {
        return r[i];
    }

    inline TIME get_base_safety_time(RESOURCE i)
    {
        return zb[i];
    }

    inline TIME get_protection_safety_time(RESOURCE i)
    {
        return zp[i];
    }
    ...
    inline DISTANCE get_base_safety_distance(RESOURCE i)
    {
        return db[i];
    }
    
    inline DISTANCE get_protection_safety_distance(RESOURCE i)
    {
        return dp[i];
    }

    inline TIME get_expiration_time(RESOURCE i)
    {
        return e[i];
    }
    ...
    inline VALUE get_value(VERTEXID v)
    {
        return w[v];
    }

    inline TIME get_resource_delay(RESOURCE i)
    {
        return delta[i];
    }

    inline TIME get_release_time(RESOURCE i)
    {
        return t[i];
    }

    inline CAPACITY get_capacity(RESOURCE i)
    {
        return c[i];
    }

    inline TIME get_optimization_horizon()
    {
        return H;
    }

    inline DISTANCE get_xy_distance()
    {
        return xy_dist;
    }

    inline COORDINATE_3D get_coordinate(VERTEXID v)
    {
    ...
    {
        int min_x = (*min_element(coordinates.begin(), coordinates.end(), [](const COORDINATE_3D &a, const COORDINATE_3D &b)
                                  { return a.x < b.x; }))
                        .x;
        int max_x = (*max_element(coordinates.begin(), coordinates.end(), [](const COORDINATE_3D &a, const COORDINATE_3D &b)
                                  { return a.x < b.x; }))
                        .x;
        return {min_x, max_x};
    }
    ...
    {
        int min_y = (*min_element(coordinates.begin(), coordinates.end(), [](const COORDINATE_3D &a, const COORDINATE_3D &b)
                                  { return a.y < b.y; }))
                        .y;
        int max_y = (*max_element(coordinates.begin(), coordinates.end(), [](const COORDINATE_3D &a, const COORDINATE_3D &b)
                                  { return a.y < b.y; }))
                        .y;
        return {min_y, max_y};
    }
    ...
    {
        int min_z = (*min_element(coordinates.begin(), coordinates.end(), [](const COORDINATE_3D &a, const COORDINATE_3D &b)
                                  { return a.z < b.z; }))
                        .z;
        int max_z = (*max_element(coordinates.begin(), coordinates.end(), [](const COORDINATE_3D &a, const COORDINATE_3D &b)
                                  { return a.z < b.z; }))
                        .z;
        return {min_z, max_z};
    }

    inline DELAYID get_delay_ID(RESOURCE i)
    {
        return resID_2_delayID[i];
    }

    inline TIME get_delay_value(DELAYID d)
    {
        return delayID_2_delayValue[d];
    }

    // The methods below are only used by Yen's algorithm to compute the kth shortest path.
    inline void restore_vertex(VERTEXID u)
    {
        G.restore_vertex(u);
    }

    inline void restore_arc(VERTEXID u, VERTEXID v)
    {
        G.restore_arc(u, v);
    }

    inline void block_vertex(VERTEXID u)
    {
        G.block_vertex(u);
    }

    inline void block_arc(VERTEXID u, VERTEXID v)
    {
        G.block_arc(u, v);
    }

    inline void restore_vertices()
    {
        G.restore_vertices();
    }

    inline void restore_arcs()
    {
        G.restore_arcs();
    }
};
